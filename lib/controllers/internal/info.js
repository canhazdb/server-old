import c from '../../constants.js';

async function infoController (context, socket, request, response) {
  const data = context.info.generators.reduce((data, generator) => {
    return {
      ...data,
      ...generator()
    };
  }, {});

  response.reply(c.STATUS_OK, {
    [c.DATA]: {
      nodeName: context.options.nodeName,
      status: context?.thisNode?.status || 'unhealthy',
      nodes: context.nodes.map(node => ({
        host: node.host,
        port: node.port
      })),
      ...data
    }
  });

  const requestData = request.json();
  if (requestData[c.DATA] && requestData[c.DATA].nodes) {
    requestData[c.DATA].nodes.forEach(node => {
      context.join(context, node.host, node.port);
    });
  }
}

export default infoController;
