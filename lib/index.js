...
async function canhazdb (rawOptions) {
  ...
  context.close = async function () {
    ...
    const workaroundTimer = setTimeout(() => {
      // This shouldn't be needed. The only time it should timeout is
      // because we can't remove the activeRequests for a client that
      // has unexpectantly disconnected.
      console.log('TODO: [FIX_001]');
