...
import crudModule from './modules/crud/index.js';
...
import raftModule from './modules/raft/index.js';
...
async function canhazdb (rawOptions) {
  ...
  async function syncNodesInfo () {
    ...
    context.syncNodesInfoTimer = setTimeout(syncNodesInfo, context.settings.infoInterval);
  }
  syncNodesInfo();
  ...
  context.close = async function () {
    ...
    const activeRequestsWait = waitUntil(() => {
    ...
    // #72 activeRequests is sometimes wrong when closing server
    // https://github.com/canhazdb/server/issues/72
    const workaroundTimer = setTimeout(() => {
      console.log('Issue 72: Workaround was required as activeRequests did not reset in time!');
      activeRequestsWait.cancel();
    }, 500);
    await activeRequestsWait;
    clearTimeout(workaroundTimer);
