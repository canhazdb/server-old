...
async function prepareOptions (rawOptions) {
  ...
  options.driver = process.env.CANHAZDB_DRIVER || rawOptions.driver || 'canhazdb-driver-nedb';
...
async function canhazdb (rawOptions) {
  ...
  if (options.joinFromDns) {
    async function dnsLookups () {
      if (scope.closed) {
        return;
      }

      const dnsLookupResults = await dns.lookup(rawOptions.joinFromDns, { all: true });
      dnsLookupResults.forEach(async item => {
        join(scope, { host: item.address, port: options.port });
      });
    }
    setInterval(dnsLookups, 5000);
  }
