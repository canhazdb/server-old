...
async function systemRaftAppendController ({ context, socket, request, response }) {
  ...
  if (!isLeader) {
    log.error('can not append to none leader');
...
async function raftModule (context) {
  ...
  raft.on('leader', function () {
    context.emit('raft:thisNodeIsLeader');
    log.info('this node has been elected as the leader', { nodeName: context.thisNode?.name });
  });
