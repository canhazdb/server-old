...
function sendToAllClients (context, command, data) {
  return Promise.all(
    context.nodes
      .filter(node => node.connected)
      .map(node => {
        return node.client.send(command, data).catch(error => {
          log.warn(error);
          return {
...
async function resolveConflict (context, conflict) {
  try {
    ...
    return sendToAllClients(context, c.CONFLICT_RESOLVE, {
      [c.INTERNAL]: true,
      [c.DATA]: conflict
    });
  } catch (error) {
...
function conflictsModule (context) {
  ...
  context.on('conflict', async function (data) {
    return sendToAllClients(context, c.CONFLICT_RAISE, {
      [c.INTERNAL]: true,
      [c.DATA]: data
    });
  });
