...
function webModule (context) {
  ...
  const server = http2.createSecureServer({
    ...
    if (urlPath.startsWith('/api/')) {
      ...
      if (request.headers.authorisation) {
        try {
          const proxyUrl = `https://localhost:${context.options.httpPort}${urlPath.replace('/api', '')}`;
          const proxiedResponse = await axios(proxyUrl, {
  ...
  server.on('listening', () => {
    log.info('web ui server listening on ' + server.address().port);
  });
