...
import internalLock from '../../controllers/internal/lock.js';
import internalUnlock from '../../controllers/internal/unlock.js';
...
import externalLock from '../../controllers/external/lock.js';
import externalUnlock from '../../controllers/external/unlock.js';
...
function controllersModule (context) {
  ...
  context.controllers.internal.add({
    command: c.LOCK,
    conditions: [],
    handler: internalLock
  });
  context.controllers.internal.add({
    command: c.UNLOCK,
    conditions: [],
    handler: internalUnlock
  });
  ...
  context.controllers.external.add({
    command: c.LOCK,
    conditions: [],
    handler: rejectWhenUnhealthy(externalLock)
  });
  context.controllers.external.add({
    command: c.UNLOCK,
    conditions: [],
    handler: rejectWhenUnhealthy(externalUnlock)
  });
