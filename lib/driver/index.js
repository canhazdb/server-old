...
async function createEjdbDriver (state) {
  ...
  return {
    count: wrapDataFunction(count),
    get: wrapDataFunction(get),
    put: wrapDataFunction(put),
    post: wrapDataFunction(post),
    patch: wrapDataFunction(patch),
    del: wrapDataFunction(del),
